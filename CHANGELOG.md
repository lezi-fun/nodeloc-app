...
> 本次更新基于 v0.3.3，重点带来 **Discourse 风格通知中心**、**Android 系统通知**和
> **聊天入口**，同时修复 onebox 图片、话题顶 / 降投票、签到和应用更新检测等问题。
> 通知从应用内延伸到了系统界面，帖子投票也改为与 Discourse Post Voting 一致的真实行为。

---

### 重点内容：你现在可以

- **从首页直接处理通知**：右上角头像显示未读数量，点击后快速浏览最近通知
- **在系统通知栏收到提醒**：点击 Android 系统通知可直达应用通知中心
- **从侧边栏进入聊天**：NodeLoc Chat 会继续使用当前登录状态
- **使用真正的顶 / 降投票**：不再显示回应列表，也不再通过长按选择表情
- **正确查看 onebox 图片**：缩略图、全尺寸图片和延迟加载图片会按实际结构展示

---

### 概览

这一版的第一条主线是通知体验。首页新增用户头像与未读角标，头像菜单、完整通知页和
Android 系统通知共享同一套通知数据；阅读单条通知只更新对应项目，“全部已读”仍由用户
主动触发。应用启动、回到前台和后台周期任务都会检查新通知，Android 13 及以上系统会在
首次打开应用时尝试请求通知权限。

第二条主线是对齐 Discourse 的真实行为。onebox 图片重新区分左侧缩略图与全宽图片，并支持
延迟加载地址；话题顶 / 降改为使用 Discourse Post Voting 接口和服务端状态，不再复用表情
回应逻辑。同时修复正式版本检测、签到请求以及正文大图展示问题。

**发布日期**：2026-09-06

**更新规模**：6 commits | 20 files changed | +1,165 / -215 lines
...
首页右上角现在显示当前用户头像和未读通知数量。点击头像会先打开原生通知菜单，
可以快速浏览最近通知，而不是直接跳转到新页面。

- 展示来源用户、头像、话题标题、发生时间和未读状态
- 点击单条通知后，仅将该通知标记为已读并跳转到对应话题
- 提供完整通知列表和由用户主动触发的“全部已读”操作
- 打开通知列表不会自动清空未读通知
- 阅读通知后会及时刷新首页未读角标

#### Android 系统通知

NodeLoc 的新通知现在可以同步显示在 Android 通知栏中。

- 点击系统通知可直接进入应用通知中心
- 多条通知会合并展示，减少通知栏占用
- 已提醒的通知会被记录，避免重复打扰
- 应用启动、回到前台以及后台周期任务都会检查新通知
- Android 13 及以上系统会在首次打开应用时尝试请求通知权限

#### 聊天入口

侧边栏新增“聊天”入口，使用当前登录状态打开 NodeLoc Chat，无需再次登录。
当前版本使用应用内网页承载聊天，后续版本将继续完善原生聊天体验。
...
修复 onebox 中包含图片时无法正确渲染的问题，并按 Discourse 的实际结构区分图片形态。

- 普通 `.aspect-image` 按左侧缩略图显示
- `.aspect-image-full-size` 按全宽大图显示
- 支持 `data-src`、`data-original-src`、`data-lazy-src` 和延迟加载 `srcset`
- 优先使用真实图片地址，避免把数据占位图当成最终图片

#### 话题顶 / 降恢复为真正的投票

顶 / 降操作现已接入 Discourse Post Voting 的投票接口和状态，不再复用表情回应逻辑。

- 不再显示回应用户列表
- 移除长按选择其他回应的操作
- 再次点击相同方向会撤销当前投票
- 切换投票方向时会正确更新票数和选中状态
- 仅在启用 Post Voting 的话题中显示投票控件

#### 应用更新提示恢复正常

- 自动构建版本改为预发布版本，不再覆盖 GitHub 上的正式最新版
- 版本比较只识别 `v1.2.3` 这类正式版本号
- 构建标签不会再被误判为比当前版本更旧或更新的正式版本

#### 签到稳定性修复

- 修正签到请求随机标识的长度
- 补齐站点来源信息，解决部分情况下签到请求被服务端拒绝的问题

#### 正文图片显示修复

正文中的大图恢复为完整图片展示，不再错误显示成缩略图加链接。

---

### 升级提醒

- 可以直接从 v0.3.3 升级到 v0.3.4，无需清除应用数据
- Android 13 及以上用户首次打开新版时，请允许通知权限以接收系统通知
- 如果此前拒绝了通知权限，可以在 Android 系统设置的应用通知页面重新开启
- Android 后台任务由系统统一调度，省电模式下通知检查可能延迟
- 当前聊天入口仍使用应用内网页，尚未提供完整原生聊天界面

---

### 下载与安装

访问 [GitHub Releases](https://github.com/lezi-fun/nodeloc-app/releases/tag/v0.3.4) 下载对应版本。
应用要求 **Android 8.0（API 26）及以上版本**。

| 文件 | 适用设备 | 建议 |
| --- | --- | --- |
| `nodeloc-universal-v0.3.4.apk` | 所有受支持设备 | **推荐**，不确定架构时选择此版本 |
| `nodeloc-arm64-v8a-v0.3.4.apk` | 大多数现代 Android 手机和平板 | ARM64 设备可优先选择 |
| `nodeloc-armeabi-v7a-v0.3.4.apk` | 较旧的 32 位 ARM 设备 | 仅旧设备需要 |
| `nodeloc-x86_64-v0.3.4.apk` | 64 位 x86 模拟器或设备 | 主要用于测试环境 |
| `nodeloc-x86-v0.3.4.apk` | 32 位 x86 模拟器或设备 | 主要用于旧测试环境 |

下载完成后可直接覆盖安装现有版本。如果系统阻止安装，请在 Android 设置中允许当前下载来源
安装未知应用。

### 完整变更

[v0.3.3...v0.3.4](https://github.com/lezi-fun/nodeloc-app/compare/v0.3.3...v0.3.4)
