...
本次更新基于 v0.3.3，新增了站内通知中心、Android 系统通知和聊天入口，
并修复签到、应用更新检测、onebox 图片显示以及话题顶 / 降投票问题。

### ✨ 新增功能

- **Discourse 风格通知中心**
  - 首页右上角显示用户头像和未读通知数量
  - 点击头像先打开原生通知菜单，可快速浏览最近通知，不再直接跳转新页面
  - 通知支持头像、来源用户、话题标题、发生时间和未读状态展示
  - 点击单条通知只将这一条标记为已读，并跳转到对应话题
  - 提供完整通知列表和用户主动触发的“全部已读”操作；打开列表不会再自动清空未读通知

- **Android 系统通知**
  - 新通知可显示在 Android 通知栏，点击后直接进入应用通知中心
  - 支持多条通知合并展示，避免相同通知重复提醒
  - 应用启动、回到前台以及后台周期任务都会检查新通知
  - Android 13 及以上系统会在首次打开应用时尝试请求通知权限

- **聊天入口**
  - 侧边栏新增“聊天”入口
  - 使用当前登录状态打开 NodeLoc Chat，无需重复登录

### 🐛 问题修复

- **onebox 图片正确显示**
  - 修复 onebox 中包含图片时无法正确渲染的问题
  - 普通缩略图和全尺寸图片会按 Discourse 的实际结构分别展示
  - 支持延迟加载图片地址，避免把占位图当成最终图片

- **话题顶 / 降恢复为真正的投票**
  - 顶 / 降现在使用 Discourse Post Voting 的投票接口和状态，不再按表情回应处理
  - 移除回应列表和长按选择回应；再次点击相同方向会撤销投票
  - 仅在启用 Post Voting 的话题中显示投票控件

- **应用更新提示恢复正常**
  - 自动构建版本改为预发布版本，不再覆盖 GitHub 上的正式最新版
  - 版本比较只识别 `v1.2.3` 这类正式版本号，避免构建标签被误判为更新版本
...
- 通知菜单与完整通知页共用统一样式和交互
- 阅读单条通知后会及时刷新首页未读角标
- 系统通知会记录已提醒的通知，减少重复打扰

### ⚠️ 已知限制

- Android 后台任务由系统统一调度，省电模式下通知检查可能延迟
- 当前聊天入口使用应用内网页承载，原生聊天界面将在后续版本继续完善
